...
-- Simp lemmas for EReal that were removed from the default simp set in Mathlib v4.29
@[simp] theorem EReal.bot_lt_top' : (⊥ : EReal) < ⊤ := bot_lt_top
@[simp] theorem EReal.bot_le_top' : (⊥ : EReal) ≤ ⊤ := bot_le
@[simp] theorem EReal.coe_le_top (x : ℝ) : (↑x : EReal) ≤ ⊤ := le_top
@[simp] theorem EReal.bot_le_coe (x : ℝ) : (⊥ : EReal) ≤ ↑x := bot_le
@[simp] theorem EReal.bot_ne_top' : (⊥ : EReal) ≠ ⊤ := bot_ne_top
@[simp] theorem EReal.top_ne_bot' : (⊤ : EReal) ≠ ⊥ := top_ne_bot
@[simp] theorem EReal.bot_le' (x : EReal) : ⊥ ≤ x := bot_le
@[simp] theorem EReal.le_top' (x : EReal) : x ≤ ⊤ := le_top
@[simp] theorem EReal.top_le_iff' (x : EReal) : ⊤ ≤ x ↔ x = ⊤ := ⟨top_le_iff.mp, fun h => h ▸ le_refl _⟩
...
theorem Example_6_2_7_eq : Example_6_2_7 \ {⊥} = (fun (x:Real) ↦ (x:EReal)) '' Example_6_2_7' := by
  ...
  · rintro ⟨⟨n, rfl⟩, -⟩
    refine ⟨n, ?_⟩
    rw [show -((↑n : EReal) + 1) = ↑(-((↑n : ℝ) + 1)) from by push_cast; rfl,
        show (-1 : EReal) + -↑n = ↑((-1 : ℝ) + -(↑n : ℝ)) from by push_cast; rfl]
    congr 1; ring
  · rintro ⟨n, h⟩
    constructor
    · refine ⟨n, ?_⟩
      ...
      rw [show -((↑n : EReal) + 1) = ↑(-((↑n : ℝ) + 1)) from by push_cast; rfl,
          show (-1 : EReal) + -↑n = ↑((-1 : ℝ) + -(↑n : ℝ)) from by push_cast; rfl]
      congr 1; ring
    · rintro rfl; simp at h; rcases h with h | h <;> exact absurd h (EReal.coe_ne_top _)
...
example : sInf Example_6_2_7 = ⊥ := by
  apply le_antisymm
  · exact sInf_le (by simp [Example_6_2_7])
  · exact bot_le
...
example : sSup Example_6_2_8 = 1 := by
  ...
  · -- 1 ≤ sSup: if sSup < 1, find an element above sSup
    ...
    · -- sSup = ↑m with m < 1
      ...
      linarith
    · rw [htop] at hlt; exact absurd hlt (not_lt.mpr le_top)
...
example : sSup Example_6_2_9 = ⊤ := by
  rw [show sSup Example_6_2_9 = ⊤ ↔ (∀ b < ⊤, ∃ a ∈ Example_6_2_9, b < a) from sSup_eq_top]
  intro b hb
  obtain ⟨ y, rfl ⟩ | rfl | rfl := EReal.def b
  . simp at hb
    obtain ⟨ n, hn ⟩ := hb
...
example (E: Set EReal) : sSup E < sInf E ↔ E = ∅ := by
  ...
  . intro h
    ...
    have h1: x ≤ sSup E := le_sSup hx
    have h2: sInf E ≤ x := sInf_le hx
...
theorem EReal.inf_ge_lower (E: Set EReal) {M:EReal} (hM: M ∈ lowerBounds E) : sInf E ≥ M := by
  rw [inf_eq_neg_sup]
