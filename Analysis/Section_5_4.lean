...
instance Real.instIsStrictOrderedRing : IsStrictOrderedRing Real where
  ...
  mul_lt_mul_of_pos_left := by
    ...
    exact pos_mul ((isPos_iff a).mpr ha) hbc

  mul_lt_mul_of_pos_right := by
    intro a ha b c hbc
    rw [antisymm] at hbc ⊢
    rw [show c * a - b * a = (c - b) * a from by ring]
    exact pos_mul hbc ((isPos_iff a).mpr ha)
...
theorem Real.rat_between {x y:Real} (hxy: x < y) : ∃ q:ℚ, x < (q:Real) ∧ (q:Real) < y := by
  ...
  . simp
    calc
      ...
      _ < (M: Real) / (N: Real) + y - x := by field_simp; linarith
      _ ≤ x + y - x := by
        gcongr
