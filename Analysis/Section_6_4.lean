...
private lemma ex648_upperseq_top (n : ℕ) : Example_6_4_8.upperseq n = ⊤ := by
  ...
  obtain ⟨M, rfl⟩ : ∃ M : ℝ, b = ↑M := by
    rcases EReal.def b with ⟨y, rfl⟩ | rfl | rfl
    · exact ⟨y, rfl⟩
    · exact absurd h (not_lt.mpr le_top)
    · exfalso; exact not_le.mpr (EReal.bot_lt_coe _)
        (hb ⟨2*↑n, by show (2*(↑n:ℤ)) ≥ max (0:ℤ) ↑n; omega, rfl⟩)
  obtain ⟨K, hK⟩ := exists_nat_gt M
  set k : ℕ := 2 * max n K
  have hk_even : Even k := ⟨max n K, by omega⟩
  have hval := ex648_even hk_even
  have hmem : (↑(k + 1 : ℝ) : EReal) ≤ ↑M :=
    hb ⟨↑k, by show (↑k:ℤ) ≥ max (0:ℤ) ↑n; omega,
        by rw [Example_6_4_8.from_eval (show (↑k:ℤ) ≥ ↑n by omega), hval]⟩
  rw [EReal.coe_le_coe_iff] at hmem
  have : (k:ℝ) ≥ K := by exact_mod_cast (show k ≥ K from by omega)
  linarith

example : Example_6_4_8.limsup = ⊤ := by
  apply eq_top_iff.mpr; show ⊤ ≤ sInf _; rw [le_sInf_iff]; rintro _ ⟨N, hN, rfl⟩; change N ≥ 0 at hN
  rw [← Int.toNat_of_nonneg hN]; exact le_of_eq (ex648_upperseq_top N.toNat).symm
...
theorem Sequence.limit_points_of_limit_points {a b:Sequence} {c:ℝ} (hab: ∀ n ≥ b.m, a.LimitPoint (b n)) (hbc: b.LimitPoint c) : a.LimitPoint c := by
  ...
  exact ⟨m, hm, calc
    ...
    _ ≤ |a.seq m - b.seq n| + |b.seq n - c| := abs_add_le _ _
