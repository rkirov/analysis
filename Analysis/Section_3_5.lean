...
theorem SetTheory.Set.mk_cartesian_inj {X Y: Set} (x x': X) (y y': Y) :
    (SetTheory.Set.mk_cartesian x y) = (SetTheory.Set.mk_cartesian x' y') ↔ x = x' ∧ y = y' := by
  constructor
  . intro h
    repeat rw [SetTheory.Set.mk_cartesian] at h
...
noncomputable abbrev SetTheory.Set.prod_commutator (X Y:Set) : X ×ˢ Y ≃ Y ×ˢ X where
  toFun := fun z ↦ SetTheory.Set.mk_cartesian (snd z) (fst z)
  invFun := fun z ↦ SetTheory.Set.mk_cartesian (snd z) (fst z)
  left_inv := by
    intro z
    simp
...
noncomputable abbrev SetTheory.Set.prod_associator (X Y Z:Set) : (X ×ˢ Y) ×ˢ Z ≃ X ×ˢ (Y ×ˢ Z) where
  toFun := fun z ↦ SetTheory.Set.mk_cartesian (fst (fst z)) (SetTheory.Set.mk_cartesian (snd (fst z)) (snd z))
  invFun := fun z ↦ SetTheory.Set.mk_cartesian (SetTheory.Set.mk_cartesian (fst z) (fst (snd z))) (snd (snd z))
...
noncomputable abbrev SetTheory.Set.iProd_equiv_prod (X: ({0,1}:Set) → Set) :
  ...
  toFun := fun x ↦
    have h := (mem_iProd _).mp x.property
    SetTheory.Set.mk_cartesian ((Classical.choose h) ⟨0, by simp⟩) ((Classical.choose h) ⟨1, by simp⟩)
  ...
  left_inv := by
    ...
    rw [Subtype.mk.injEq]
    rw [hf]
    simp only [fst_of_mk_cartesian, snd_of_mk_cartesian, coe_of_fun_inj]
    funext i
...
noncomputable abbrev SetTheory.Set.iProd_equiv_prod_triple (X: ({0,1,2}:Set) → Set) :
  ...
  toFun := fun z ↦
    ...
    SetTheory.Set.mk_cartesian ((Classical.choose h) ⟨0, by simp⟩)
      (SetTheory.Set.mk_cartesian ((Classical.choose h) ⟨1, by simp⟩) ((Classical.choose h) ⟨2, by simp⟩))
...
theorem SetTheory.Set.direct_sum {X Y Z:Set} (f: Z → X) (g: Z → Y) :
  ...
  . intro h1 h2 h
    ...
    simp at ha2
    rw [ha1] at h1c h2c
    simp only [ha2] at h1c h2c
    · simp only [fst_eval, snd_eval] at h1c h2c
      exact ⟨congrArg Subtype.val h1c.symm, congrArg Subtype.val h2c.symm⟩
    · rw [ha2] at p2; exact p2
    · rw [ha1] at p1; exact p1
...
theorem SetTheory.Set.iUnion_inter_iUnion {I J: Set} (A: I → Set) (B: J → Set) :
  ...
  . intro h
    ...
    rw [mem_inter]
    constructor
    . simp [fst_eval]; exact hi1
    . simp [snd_eval]; exact hj2
  . intro h
    rw [mem_iUnion] at h
    obtain ⟨ p, hp ⟩ := h
    rw [mem_inter] at hp
  ...
  . use fun x ↦
    ...
    . intro hx
      ...
      have h2 : OrderedPair.toObject { fst := ↑(fst ⟨x, h⟩), snd := snd ⟨x, h⟩ } ∈ G := by
        ...
        constructor
        . subst h'; simp [fst_eval]
        . subst h'; simp [snd_eval]
    ...
    . intro h
      ...
      rw [OrderedPair.toObject_eq]
      subst h'
      exact ⟨congrArg Subtype.val (fst_eval _ _), congrArg Subtype.val (snd_eval _ _)⟩
  . intro f f' h h'
    rw [graph] at h h'
    ...
    obtain ⟨ h1, h2 ⟩ := hf
    obtain ⟨ h1', h2' ⟩ := hf'
    simp only [fst_eval, snd_eval] at h2 h2'
    exact congrArg Subtype.val (h2.symm ▸ h2'.symm ▸ rfl)
  ...
  . let g := (Y ×ˢ X).powerset.specify (fun T ↦
    ...
    . intro h
      ...
      have : set_to_object (graph T) ∈ g := by
        simp only [g]
        rw [specification_axiom'']
        have hmem : set_to_object (graph T) ∈ (Y ×ˢ X).powerset := by
          rw [mem_powerset]; use graph T; simp only [true_and]; rw [graph]; apply specification_axiom
        exact ⟨hmem, fun y => by
          apply existsUnique_of_exists_of_unique
          · use T y
            generalize_proofs a
            ...
            rw [← h1]; rw [graph]; rw [specification_axiom'']
            simp [fst_eval, snd_eval]
          · intro x x' hx hx'
        ...
        . let ap := choose ih
          ...
          . simp
            have hN : nat_equiv.symm 0 ≠ N + 1 := by
              rw [nat_equiv_symm_zero]
              simp only [ne_eq, Nat.right_eq_add, Nat.add_eq_zero_iff, one_ne_zero, and_false,
                not_false_eq_true]
            exact ap_spec.1.1
          . intro n hn
      ...
      . intro x1 x2 h
        ...
        . rw [Equiv.symm_apply_eq] at hi
          ...
          . rw [Equiv.symm_apply_eq] at hi2
            ...
            have hn12feq := not_imp_not.mp this h
