...
theorem abs_div (x y:ℚ) : |x / y| = |x| / |y| := by
  ...
  . have hy': |y| ≠ 0 := by
      intro h; apply hy; exact (abs_eq_zero_iff y).mp h
    field_simp
    rw [← abs_mul]
    field_simp
...
theorem pow_sub (x:ℚ) (m n:ℕ) (h: n ≥ m) (hx: x ≠ 0): x ^ n / x ^ m = x ^ (n - m) := by
  ...
  . apply eq_zero_of_pow_eq_zero at hm
    tauto
  . field_simp [hm]
    rw [pow_add, add_comm, Nat.sub_add_cancel h]
...
theorem zpow_mul (x:ℚ) (n m:ℤ) : (x^n)^m = x^(n*m) := by
  ...
  · obtain ⟨n', hn⟩ := Int.eq_nat_or_neg n
    ...
    · simp only [zpow_neg, zpow_natCast, mul_neg, one_div]
      rw [show ((↑n' * ↑m' : ℤ)) = ((↑(n' * m') : ℤ)) from by push_cast; ring]
