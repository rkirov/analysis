...
theorem Rat.countablyInfinite : CountablyInfinite ℚ := by
  ...
  rw [h] at this
  replace := (AtMostCountable.equiv (EqualCard.univ _)).mp this
  simp [AtMostCountable] at this
...
/-- Exercise 8.1.10.  Note the lack of the `noncomputable` keyword in the {kw}`abbrev`.
