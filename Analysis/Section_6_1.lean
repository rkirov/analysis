...
theorem Sequence.tendsTo_mul {a b:Sequence} {L M:ℝ} (ha: a.TendsTo L) (hb: b.TendsTo M) :
  ...
  calc
    ...
    _ ≤ |a n * b n - a n * M| + |a n * M - L * M| := by exact abs_add_le _ _
    ...
    _ = ε / 2 + |M| * (ε / (2 * (1 + |M|))) := by
      ...
      field_simp [hBneg]
    _ ≤ ε / 2 + ε / 2 := by gcongr; field_simp; linarith [abs_nonneg M]
    _ = ε := by ring
...
/-- Theorem 6.1.19(c) (limit laws).  The {lean}`Sequence.TendsTo` version is more usable than the {lean}`lim` version
    in applications. -/
