  ...
  . rcases hm' with ⟨rfl | rfl⟩
    ...
    . subst m -- why didn't rfl do it
      ...
      . have : (n': ℤ) + -(m':ℤ) = -(m' - n') := by omega
        ...
        field_simp
        norm_cast
        conv_rhs => rw [show m' = n' + (m' - n') from (Nat.add_sub_cancel' h2.le).symm]
        rw [pow_add]
  . subst n -- why didn't rfl do it
    ...
    . field_simp
      ...
      . have : -(n': ℤ) + (m':ℤ) = -(n' - m') := by omega
        ...
        rw [zpow_neg]
        field_simp
        conv_rhs => rw [show n' = m' + (n' - m') from (Nat.add_sub_cancel' h2).symm]
        rw [pow_add]
      . have : -(n': ℤ) + (m':ℤ) = m' - n' := by omega
        rw [this]
        simp at h2
        ...
        rw [pow_eq_pow]
        norm_cast
        conv_lhs => rw [show m' = n' + (m' - n') from (Nat.add_sub_cancel' h2.le).symm]
        rw [pow_add]
    . subst m
      rw [zpow_neg]
      field_simp
...
theorem Real.zpow_mul (x:Real) (n m:ℤ) : (x^n)^m = x^(n*m) := by
  by_cases hx : x = 0
  · subst hx
    rcases eq_or_ne n 0 with rfl | hn <;> rcases eq_or_ne m 0 with rfl | hm <;>
      simp_all [zero_zpow]
  · obtain ⟨n', hn⟩ := Int.eq_nat_or_neg n
    obtain ⟨m', hm⟩ := Int.eq_nat_or_neg m
    have hcast : (↑n' * ↑m' : ℤ) = ↑(n' * m') := by push_cast; ring
    rcases hn with rfl | rfl <;> rcases hm with rfl | rfl
    · norm_cast; exact pow_mul x m' n'
    · simp only [zpow_neg, zpow_natCast, mul_neg, one_div]
      rw [hcast]
      simp only [one_div, zpow_neg]
      congr 1; exact pow_mul x m' n'
    · simp only [zpow_neg, zpow_natCast, neg_mul, one_div]
      rw [inv_pow, hcast]
      simp only [one_div, zpow_neg]
      congr 1; exact pow_mul x m' n'
    · simp only [zpow_neg, neg_mul, mul_neg, neg_neg, one_div]
      rw [inv_pow, inv_inv]
      norm_cast; exact pow_mul x m' n'
...
theorem Real.root_mono_of_gt_one {x : Real} (hx: x > 1) {k l: ℕ} (hkl: k > l) (hl: l ≥ 1) : x.root k < x.root l := by
  set xl := x.root l
  have h1 : xl ^ l = x :=
    (Real.eq_root_iff_pow_eq (by linarith) (Real.root_nonneg (by linarith) (by linarith)) (by linarith)).mp rfl
  set xk := x.root k
  have h2 : xk ^ k = x :=
    (Real.eq_root_iff_pow_eq (by linarith) (Real.root_nonneg (by linarith) (by linarith)) (by linarith)).mp rfl
  rw [← h1] at h2
...
theorem Real.ratPow_add {x:Real} (hx: x > 0) (q r:ℚ) : x^(q+r) = x^q * x^r := by
  ...
  conv_rhs => rw [show (a:ℚ) = ((a * d):ℤ) / d by field_simp; push_cast; ring]
  rw [ratPow_def hrb _ hd]

  have hrd: x.root d > 0 := by exact (Real.root_pos hx.le hd).mpr hx
  rw [← ratPow_eq_pow hrd c]
  conv_rhs => rw [show (c:ℚ) = ((c * b):ℤ) / b by field_simp; push_cast; ring]
