...
- Prefer `use` over `refine` when providing existential witnesses or constructor arguments.
...
- **Avoid unnecessary `show ... from`.**  Don't write `show X from term` when `term` already has type `X` — just write `term`. The `show` wrapper is only needed when providing a type annotation for a tactic block (`show X from by tac`) where the tactic can't infer the goal.
