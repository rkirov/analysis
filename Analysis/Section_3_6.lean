...
theorem SetTheory.Set.Remark_3_6_6 (n:ℕ) :
  ...
  . intro y
    ...
    use ⟨n', by
      ...
      use n'
      simp only [and_true]
      simp_all only [Nat.succ_eq_add_one, le_add_iff_nonneg_left, zero_le, ne_eq, Nat.add_eq_zero_iff, one_ne_zero,
  ...
  . have := pos_card_nonempty (by simp) hX
    ...
    . have hneq : x ∉ (X' ∪ Y) := by aesop
      ...
      have hx'ufin : (X' ∪ Y).finite := by
        by_contra h0
        have : (X' ∪ Y).card = 0 := by
          unfold card; exact dif_neg h0
        rw [this] at hn
        have hXc := has_card_to_card hX'
        have hYc := has_card_to_card hY
        rw [hXc, hYc] at hn
        symm at hn
        rw [Nat.add_eq_zero_iff] at hn
        obtain ⟨ hn0, hn1 ⟩ := hn
        rw [hn0] at hX'
        rw [hn1] at hY
...
/--
  ...
  - use induction on {lean}`n`
  - for the induction step, use cases on whether {lean}`f` is surjective or not
  - if {lean}`f` is not surjective, then we can find a gap in the range of {lean}`f` and define f'
    to map that gap to the value {lean}`n` and use induction on {lean}`n`.
  - if {lean}`f` is surjective, then it is already the bijection.
...
/--
  Using {name}`Function.surjInv` requires axiom of choice.
